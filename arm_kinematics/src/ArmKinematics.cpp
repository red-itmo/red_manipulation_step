...
Vector3d ArmKinematics::ForwardKin(const JointValues &angles)
{
	...
	d1vec(1) = 0;
	d1vec(2) = d1z;
	double theta = angles(1) + angles(2) + angles(3);
	Vector3d plane;
...
JointValues ArmKinematics::numericalIK(const Pose &pose, const JointValues &maxAngle)
{
	...
	while (difference.norm() > 10e-10 && iter < iter_num)
	{
		...
		FKoutput = ForwardKin(iterMaxAngle);
		matrixFK(0) = FKoutput(0);
		matrixFK(1) = FKoutput(1);
		matrixFK(2) = FKoutput(2);
		matrixFK(3) = iterMaxAngle(1) + iterMaxAngle(2) + iterMaxAngle(3);
	...
	if (iter > iter_num - 1)
	{
		iterMaxAngle.setAll(-1000);
		// ROS_FATAL("[ArmKinematics]Solution can't be found");
		return iterMaxAngle;
	}
	if (!checkAngles(iterMaxAngle))
		return 0;
	for (int i = 0; i < 5; ++i)
		iterMaxAngle(i) = round(iterMaxAngle(i) * 10000) / 10000;
	return iterMaxAngle;
}
